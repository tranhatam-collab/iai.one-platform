...
payload_file="${1:-docs/ops/scripts/legacy_users_12.sample.json}"
...
curl -sS "${API_BASE}/v1/migration/health" -H "Authorization: Bearer ${ADMIN_SECRET}" | tee /tmp/migration_users_health_run.json
echo
...
out = '/tmp/migration_users_batch_result.json'
...
echo
echo "==> Result file: /tmp/migration_users_batch_result.json"
